...
DEFAULT_DLSHOGI_DIR = Path(r"/home/daigo/デスクトップ/DeepLearningShogi")
DEFAULT_TRAIN_DIR = Path(r"/media/daigo/36AEC093AEC04CD5/hcpe/")
DEFAULT_TEST_DATA = Path(
    r"/home/daigo/デスクトップ/fg.hcpe"
)
DEFAULT_MODEL_ROOT = Path(r"/home/daigo/デスクトップ/model")
...
"trainer": {
    ...
    # Multi GPU
    # =====================================================
    "devices": args.devices if args.devices else 1,
...
def run_one_round(
    ...
    *,
    ...
    try:
        cache_root = out_dir / "_ti"

        if cache_root.exists():
            import shutil

            print(f"cleanup compile cache: {cache_root}")
...
def main() -> None:
    ...
    nargs="+",
    default=[0],
    help="GPU device ids for PTL/DDP training",
    )
    ...
    if args.show_log:
        log_target = (
            ...
            if args.out_dir
            else make_out_dir(model_root, args.network)
        )

        show_train_log(
            [log_target],
            teacher_root=train_dir.parent,
        )
    ...
    rng = random.Random(0)
    rng.shuffle(teacher_files)

    if not teacher_files:
        raise FileNotFoundError(
            f"No .hcpe/.hcpe3 files found in {train_dir}"
        )
